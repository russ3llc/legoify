...
from util import get_lego_colors, color_name_to_rgb, black_or_white
...
def pixelate(image_path, height, width=None, dither=None):
    ...
    lego_pal = Pal.from_rgb(get_lego_colors(True))
...
def get_color_filter(in_image, color):
    ...
    bg_color = 0 if black_or_white(color) == [0, 0, 0] else 255
